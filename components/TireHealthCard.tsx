...
import { Activity, CalendarClock, Route, Disc } from 'lucide-react';
...
export const TyreHealthCard: React.FC<Props> = ({ data, properties }) => {
  ...
  return (
    <div className={`relative overflow-hidden rounded-2xl border ${config.border} bg-slate-900 shadow-2xl transition-all duration-500 h-full flex flex-col`}>
      ...
      <div className="relative p-6 z-10 flex flex-col h-full">
        ...
        <div className="flex justify-center mb-6">
          <div className={`w-40 h-40 rounded-full shadow-2xl border-4 border-slate-800 flex items-center justify-center relative transition-colors duration-1000 ${config.tireTint}`}>
             {/* Bloom Overlay */}
             <div 
             ...
             {/* Tyre Texture Rings */}
             <div className="w-28 h-28 rounded-full border border-white/5 opacity-20" />
             <div className="absolute w-36 h-36 rounded-full border border-white/5 opacity-20" />
             
             {/* Central Hub */}
             <div className="w-14 h-14 rounded-full bg-slate-950 border border-slate-800 flex items-center justify-center z-10">
